...
app.post('/api/ai/prd', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const existingCardsStr = existingCards.length > 0 ? `\n\n=== HISTÓRIAS OBRIGATÓRIAS A SEREM REFINADAS (${existingCards.length} histórias) ===\n${existingCards.map(c => `ID: ${c.id} | Título: ${c.title} | Contexto: ${c.description}`).join(`\n`)}\n\nATENÇÃO: Você é obrigado a retornar exatamente ${existingCards.length} objetos no array "refinedStories". O campo "card_id" de cada objeto DEVE corresponder aos IDs listados acima. Não mescle histórias, não altere o escopo para excluir nenhuma, e não invente IDs novos.` : "";

    const taskDescription = existingCards.length > 0
      ? `**Task:** Você está recebendo um conjunto de ${existingCards.length} histórias pré-definidas (extraídas na fase de importação). O seu objetivo único e exclusivo é REFINAR CADA UMA dessas ${existingCards.length} histórias exaustivamente. Para cada história fornecida, preencha todos os campos avançados: visão do usuário, narrativa de negócio, Critérios de Aceite (BDD), resumo, cenários de teste em três níveis, estimativa, detalhamento das tasks, análise de riscos. VOCÊ DEVE DEVOLVER EXATAMENTE ${existingCards.length} HISTÓRIAS, CADA UMA COM SEU "card_id" ORIGINAL INTACTO.`
